...
class _RecommendationPageState extends State<RecommendationPage> {
  ...
  void initState() {
    super.initState();
    _log.info(
        "RecommendationPage initState: query='${widget.query}', category='${widget.category}', mood='${widget.mood}'");
  ...
  void didUpdateWidget(covariant RecommendationPage oldWidget) {
    ...
    if (oldWidget.query != widget.query ||
      ...
      _log.info(
          "Query parameters changed in didUpdateWidget. Fetching new recommendations.");
      _fetchRecommendations();
    } else {
      _log.info(
          "Query parameters have not changed. Skipping fetch in didUpdateWidget.");
    }
  }
  ...
  Future<void> _fetchRecommendations() async {
    ...
    // --- 核心修改：移除對 isInitialized 的檢查 ---
    final backendUrlService =
        Provider.of<BackendUrlService>(context, listen: false);
    ...
    try {
      ...
      var uri = Uri.parse('$baseUrl/recipes/recommend')
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            _recommendedRecipes =
                jsonData.map((data) => RecipeListItem.fromJson(data)).toList();
          });
        }
      } else {
  ...
  Future<void> _handleRecipeTap(RecipeListItem recipeListItem) async {
    ...
    _log.info(
        "--- Starting _handleRecipeTap for: ${recipeListItem.recipeName} ---");
    ...
    try {
      ...
      final detailsUri = Uri.parse(
          '$baseUrl/recipe/${Uri.encodeComponent(recipeListItem.recipeName)}');
      _log.info("Fetching details from: $detailsUri");
      final detailsResponse =
          await http.get(detailsUri).timeout(const Duration(seconds: 15));
      ...
      final Map<String, dynamic> detailsJson =
          jsonDecode(utf8.decode(detailsResponse.bodyBytes));
      _log.info("Details JSON parsed successfully.");

      // 2. 獲取食譜步驟
      final stepsUri = Uri.parse(
          '$baseUrl/recipe/${Uri.encodeComponent(recipeListItem.recipeName)}/steps');
      _log.info("Fetching steps from: $stepsUri");
      final stepsResponse =
          await http.get(stepsUri).timeout(const Duration(seconds: 15));
      ...
      final List<dynamic> stepsJson =
          jsonDecode(utf8.decode(stepsResponse.bodyBytes));
      _log.info(
          "Steps JSON parsed successfully. Step count: ${stepsJson.length}");

      // 3. 組合數據並顯示彈窗
      final RecipeDetails completeDetails =
          RecipeDetails.fromCombinedJson(detailsJson, stepsJson);
  ...
  Future<void> _showRecipeDetailsDialog(RecipeDetails details) async {
    ...
    if (details.requiredItemsText != null &&
        details.requiredItemsText!.isNotEmpty) {
      detailPages
          .add(_buildInfoCardPage("必備原料和工具", details.requiredItemsText!));
    }
    if (details.calculationsText != null &&
        details.calculationsText!.isNotEmpty) {
      detailPages
          .add(_buildInfoCardPage("精確用量 (計算)", details.calculationsText!));
    }
    ...
    if (detailPages.isEmpty) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('此食譜暫無詳細資訊可供展示。')));
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                  ...
                  padding: const EdgeInsets.symmetric(
                      horizontal: 20, vertical: 12) // 調整按鈕 padding
                  ),
              onPressed: (_isFetchingDetails || details.steps.isEmpty)
                  ? null
                  : () {
          ...
          children: [
            Text(
              ...
              style: TextStyle(
                  ...
                  color: Colors.deepOrange[700]), // 標題字體加大
            ),
    ...
    return Scaffold(
      body: _isLoadingRecipes
          ...
          : _recommendedRecipes.isEmpty
              ...
              : AnimationLimiter(
                  // <--- 加入 AnimationLimiter 包裹 ListView
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String imagePath =
                          getRecipeImagePath(recipeItem.recipeName);
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(
                                                height: 6.0), // 名稱和描述/難度之間的間距

                                            // --- 修改：將難度和分類放在一行或相鄰顯示 ---
                                            Row(
                                              // 使用 Row 並排顯示難度和第一個分類 (如果有的話)
                                              crossAxisAlignment:
